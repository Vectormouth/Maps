...
// GET /test → para probar desde navegador
app.get('/test', async (req, res) => {
  ...
  const body = {
    ...
    openNow,
    languageCode: lang,
    // ✅ searchText usa locationBias, no locationRestriction
    locationBias: {
      circle: {
...
// POST /api/dentists → usado por el frontend
app.post('/api/dentists', async (req, res) => {
  try {
    ...
    const body = {
      ...
      openNow: !!openNow,
      languageCode: lang,
      // ✅ también aquí usamos locationBias
      locationBias: {
        circle: {
          center: { latitude: lat, longitude: lng },
